            ...
            if (collision.collider == targetCollider)
            {
                ...
                if (collision.collider.Raycast(ray, out hit, 2))
                {
                    if (colorSpawnedObject)
                    {
                        ...
                        {
                            color = Color.green;
                            if(buttonTaizi.transform.position.y < 0.6f) buttonTaizi.transform.position += new Vector3(0, 1.2f, 0);
                            Debug.Log(buttonTaizi.transform.position.x + " " + buttonTaizi.transform.position.y + " " + buttonTaizi.transform.position.z);
                        }
                        else
                            color = Color.yellow;
